...
const Estimacion = () => {
  ...
  const [estimaciones,setEstimaciones] = useState<EstimacionCosechaInterface[]>([]);
  ...
  const obtenerProductorPorId = (id: number): ProductorInterface | undefined => {
  ...
  return (
    <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-2">
      {estimaciones.map((estimacion) => {
        const productor = obtenerProductorPorId(Number(estimacion.parcela.ID_productor));
        return(
          <EstimacionCard productor={productor} key={estimacion.id} estimacionCosecha={estimacion} />
        )  
})}
    </div>
  )
