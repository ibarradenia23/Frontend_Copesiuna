...
const TiposCultivosForm: React.FC<TiposCultivosPropsInterface> = ({
  tipoCultivo,
}) => {
  const {
    register,
  ...
  useEffect(() => {
    ...
    if (isErrorEditar) {
      setToast({
        type: "error",
        message:
          "Error al editar el tipo de cultivo: " +
            ...
            <input
              ...
              {...register("edad", {
                ...
                minLength: {
                  value: 2,
                  message:
                    "La edad del cultivo debe tener al menos 2 caracteres",
                },
              })}
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-[#016F35] block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
              placeholder="8 meses"
