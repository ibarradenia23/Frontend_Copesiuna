...
import {
  useEliminarTipoCultivo,
  useObtenerTiposCultivos,
} from "../hooks/useTiposCutivos";
...
const CartasCultivos = () => {
  ...
  const { data: tiposCultivosResponse /*isLoading*/ } =
    useObtenerTiposCultivos();
  const {
    mutate: eliminarTiposCultivos,
    isError,
    isSuccess,
    error,
  } = useEliminarTipoCultivo();
  const [tiposCultivosD, setTiposCultivosD] = useState<
    TiposCultivosInterface[]
  >([]);
  ...
  return (
    <div className="p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 min-h-128">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          className="text-white inline-flex items-center bg-primary hover:bg-[#016F35] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary dark:hover:bg-[#016F35] dark:focus:ring-primary justify-center mt-4 gap-2"
      ...
      <div className="contenedorcartas mt-4">
        {tiposCultivosD &&
          tiposCultivosD?.map((tipoCultivo) => (
            <div className="p-6 mb-4 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 flex justify-between items-center">
              <div className="">
                ...
                <span className="flex text-gray-700 dark:text-gray-400 items-center">
                  <Clock className="inline mr-1 h-4 w-4" /> Edad:{" "}
                  {tipoCultivo.edad}
                </span>
              </div>
              <div className="btones flex gap-2">
                ...
                <button
                  onClick={() => handleEliminar(tipoCultivo.id ?? 0)}
                  className=" items-center  font-medium rounded-lg text-sm p-2.5 text-center w-full justify-center text-white bg-error hover:bg-red-900 focus:ring-4 focus:ring-gray-100   dark:bg-error dark:hover:bg-red-900 dark:focus:ring-error"
                >
